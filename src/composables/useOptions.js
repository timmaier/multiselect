...
export default function useOptions(props, context, dep) {
    ...
    if (groupped.value) {
      ...
      groups.forEach((group) => {
        optionsToArray(group[groupOptions.value]).forEach((option) => {
          eo.push(
            Object.assign({}, option, group.disabled ? { disabled: true } : {})
          );
        });
      });
  ...
  const fg = computed(() => {
    ...
    return filterGroups(
      (ro.value || /* istanbul ignore next */ []).map((group) => {
        const arrayOptions = optionsToArray(group[groupOptions.value]);

        return {
          ...group,
          group: true,
          [groupOptions.value]: filterOptions(arrayOptions, false).map((o) =>
            Object.assign({}, o, group.disabled ? { disabled: true } : {})
          ),
          __VISIBLE__: filterOptions(arrayOptions).map((o) =>
            Object.assign({}, o, group.disabled ? { disabled: true } : {})
          ),
        };
  ...
  const noResults = computed(() => {
    return (
      eo.value.length > 0 &&
      fo.value.length == 0 &&
      ((search.value && groupped.value) || !groupped.value)
    );
  });
      ...
      case "multiple":
        update(
          Array.isArray(option)
            ...
            : iv.value.filter(
                (v) => v[valueProp.value] != option[valueProp.value]
              )
        );
        break;
    }
  ...
  const isSelected = (option) => {
    if (option.group !== undefined) {
      return mode.value === "single"
        ? false
        : areAllSelected(option[groupOptions.value]) &&
            option[groupOptions.value].length;
    }
          ...
          select(
            group[groupOptions.value]
              .filter(
                (o) =>
                  iv.value
                    .map((v) => v[valueProp.value])
                    .indexOf(o[valueProp.value]) === -1
              )
              .filter((o) => !o.disabled)
              .filter(
                (o, k) =>
                  iv.value.length + 1 + k <= max.value || max.value === -1
              )
          );
        }
        break;
  ...
  const shouldHideOption = (option) => {
    return (
      ["tags", "multiple"].indexOf(mode.value) !== -1 &&
      hideSelected.value &&
      isSelected(option)
    );
  };
  ...
  const filterGroups = (groups) => {
    ...
    return groupHideEmpty.value
      ? groups.filter((g) =>
          search.value ? g.__VISIBLE__.length : g[groupOptions.value].length
        )
  ...
  const filterOptions = (options, excludeHideSelected = true) => {
    ...
    if (search.value && filterResults.value) {
      fo = fo.filter((option) => {
        return (
          normalize(option[trackBy.value], strict.value).indexOf(
            normalize(search.value, strict.value)
          ) !== -1
        );
      });
    }
